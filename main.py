...
class CreditsDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        creator_label = QLabel("Realizzato da: Matteo Di Maria \n\n Laureando del Corso di Laurea in Ingegneria Informatica e dell\'Automazione")
        ...
        for widget in widgets_to_collect:
            if isinstance(widget, QLineEdit):
                if widget.text().strip():
                    if widget.objectName() == "gnb_ip":
                        params['gnb_ip_ngap'] = widget.text().strip()
                        params['gnb_ip_ngu'] = widget.text().strip()
                    else:
